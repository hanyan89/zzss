...
import com.buzhidao.zzss.request.DefaultSslSocketFactory;
import com.buzhidao.zzss.response.MagnetItem;
    ...
    public List<MagnetItem> parser(MagnetRule rule, String keyword, String sort, int page, String userAgent) throws MagnetParserException, IOException {
        ...
        */
        ...
        for (Map.Entry<String, String> entry : headers.entrySet()) {
            ...
            logsb.append(header);
            logsb.append(":");
            logsb.append(headers.get(header));
            logsb.append("\n");
        }
        ...
        try {
            ...
            if (CacheManager.getInstance().cacheExists(cacheName)) {
                ...
                if (element == null) {
                    ...
                    log.info(String.format("成功预加载 %s-%s-%d，缓存%d条数据", current.getSite(), current.getKeyword(), page, items.size()));
                }
            }
