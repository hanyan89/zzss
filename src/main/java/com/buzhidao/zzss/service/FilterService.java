...
public class FilterService {
    ...
    public void init() {
        ...
        log.info(String.format("过滤词文件 %s--->%s--->加载%d个过滤词", filterFile.getAbsolutePath(), String.valueOf(filterFile.exists()), mFilterList.size()));
    }
    ...
    public boolean add(String input) throws Exception {
        ...
        try {
            ...
            completed = true;
            log.info(String.format("屏蔽词添加成功--->%s--->当前%d个屏蔽词", input, mFilterList.size()));
            return true;
        } catch (Exception e) {
            completed = true;
            log.error("屏蔽词添加失败", e);
